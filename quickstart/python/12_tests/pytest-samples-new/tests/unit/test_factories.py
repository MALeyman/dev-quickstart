...
def test_stub_factory():
    # конструктор
    my_object = ObjectFactory() # без передачи параметров
    assert my_object.name == "value"

    another_object = ObjectFactory(name="Guido") # явно значение для свойства name
    assert another_object.name == "Guido"


def test_dict_factory():
    # соответствие объектов типу
...
def test_batch_factories():
    """
    функции SimpleFactory для создания объектов 
    в пакетах in batches
    Тест конструктора
    """

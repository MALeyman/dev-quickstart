...
import arrow
'''
Factory - это основной класс фабрики в factory_boy.
Faker используется для генерации случайных данных, похожих на реальные.
LazyAttribute позволяет создавать атрибуты, которые вычисляются 
динамически при каждом вызове фабрики.
LazyFunction аналогичен LazyAttribute, но работает с функциями.
SubFactory позволяет использовать одну фабрику внутри другой.
'''
from factory import Factory, Faker, LazyAttribute, LazyFunction, SubFactory

'''
FuzzyChoice используется для выбора случайного элемента из списка.
FuzzyDateTime генерирует случайные даты и времени в заданном диапазоне.
FuzzyFloat генерирует случайные плавающие точки чисел в заданном диапазоне.
FuzzyText генерирует случайный текст в заданном формате.
'''
...
fake_it = FakeIt()  # Еще один способ использовать Faker
...
class Inner(NamedTuple):
    '''
    неизменяемые структуры данных с именованными полями
    '''
    inner_value: float
...
class Structure(NamedTuple):
    # Это может быть класс данных, модель Pydantic или класс Marshmallow
    # Также может быть модель SQLAlchemy, Django или PyMongo

...
import aiohttp # выполнять асинхронные HTTP-запросы
# с текущего года https://github.com/ultrafunkamsterdam/undetected-chromedriver

all_data = []

async def get_page_data(session, category: str, page_id: int):
    if page_id:
        url = f'https://www.ozon.ru/category/{category}/?page={page_id}'
    else:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for cat in categories_list:
            for page_id in range(100):
                ...
                tasks.append(task)
        await asyncio.gather(*tasks) # для группировки и ожидания завершения нескольких корутин
